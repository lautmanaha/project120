...
### תנודתיות ורשימות חדשות (המלצות ויקטור טיסה, 9.2026)
- **prior היררכי על התנודתיות** (`--walk-pool-sd 0.5`, ברירת מחדל): לכל מפלגה סיגמה משלה להליכה האקראית, אבל הסיגמות נמשכות
  לרמה ענפית משותפת שנלמדת מהנתונים. החליף את תקרת ה-tanh (`--walk-sigma-max`, עכשיו 0 = כבוי) שהייתה קלמפ ידני.
- **מסכת כניסה** (`--entry-skip-weeks 1`, ברירת מחדל): תא (סקר, מפלגה) שהוא השלמה אוטומטית (`imputed:` ב-pct_source), או
  ששייך לרשימה מאוחרת (לא דווחה ברוב הסקרים בשבועיים הראשונים) ונמצא לפני מועד הכניסה + N שבועות - לא נכנס לנראות.
  המימוש ב-`vendor/kronikas/model.py` (`PollData.mask`): הסקר נמדל על תת-הסימפלקס של המפלגות שהוא כן דיווח עליהן
  (תת-וקטור מנורמל של דיריכלה הוא דיריכלה - מדויק). המפלגה מאותחלת מהסקרים הראשונים שדיווחו עליה.
  האתר מציג את המסלול והנקודות של רשימה מאוחרת רק ממועד הכניסה (`forecast.json -> entry_mask`).
- בדיקת עבר 2022: שלוש התצורות (תקרה / היררכי / חופשי) זהות - ב-2022 אף רשימה לא נכנסה אחרי הגשת הרשימות.
  ב-2026 ההבדל כולו בעמך ישראל (נכנסה לסקרים כרשימה ב-26.8): בלי מסכה, prior היררכי לבד מנפח את התנודתיות שלה
  ל-0.26/שבוע וטווח 1-10%; עם מסכה: 0.05/שבוע, 3.4% (2.5-4.5).
- ניסויים: `python model/backtest_2022.py --tag NAME -- <דגלים ל-run_model.py>` כותב ל-`backtest_2022_NAME.json`.

### כיול רב-מערכתי (2019א-2022)
- `data/hist/wiki_*.json` - טבלאות הסקרים של 2019א, 2019ב, 2020, 2021 (ויקיפדיה האנגלית; מנדטים; מפלגות מתחת לסף באחוזים).
- `model/hist_calibration.py` -> `model/calibration_hist.json`: טעות הענף (ממוצע הסקרים ב-3 השבועות האחרונים מול התוצאה)
  לכל מערכת, לפי "משפחות" (הליכוד, ש"ס, יהדות התורה, ימין דתי, ישראל ביתנו, המשותפת, רע"ם, שמאל, מרכז); מפלגה שנפלה
  בסף נמדדת לפי אחוז הקולות (x1.25 = שווה-ערך מנדטים) ולא לפי 0. `combined`: ממוצע משוקלל בדעיכה מעריכית (זמן מחצית
  3 שנים) + סטיית תקן אמפירית בין המערכות.
- `model/anchor.py` ממפה את המשפחות למפלגות 2026 (ימין דתי -> הציונות הדתית/עוצמה יהודית לפי גודל; שמאל -> הדמוקרטים;
  מרכז -> ישר/ביחד/כחול לבן) וכותב ל-`anchor_trusted.json`: `hist_mean`, `hist_sd`, `combined_hist` (= hist_mean + מחצית
  הפער למכונים המהימנים).
- החלטות (17.9.2026): הצינור רץ על `combined_hist` = כיול רב-מערכתי + מחצית הפער למכונים המהימנים; מחצית הפער נכנסת
  כממוצע עם סטיית תקן בגודלה (הנחה, לא מדידה); הימין הדתי נמדד רק על 2021-2022.
- תיקון ב-`vendor/kronikas/model.py`: ה-prior של המצב ההתחלתי (eta_init) ממורכז על הערך מתוקן-ההטיה. בלעדיו ה-prior,
  שמעוגן על הסקרים הראשונים כמו שהם, מושך הטיה עם סטיית תקן רחבה בשקט לאפס (ליכוד קיבל -0.8 במקום -4.8).
- הפעלה: `run_model.py --industry-lean-key combined_hist --industry-sd-key hist_sd` (בלי `--industry-sd-key`: הנוסחה
  הישנה, 12% מגודל המפלגה). ההחלטה איזה מפתח משמש את הצינור נמצאת ב-`scraper/daily_update.py`.
...
### בקרת איכות (הסגר)
כל סקר שחולץ עובר `scraper/qa_check.py` לפני שהוא נכנס למודל: מכון חדש, שורה שמופתה ל"אחר" עם >3%, מפלגה גדולה חסרה,
מפלגה כפולה, סכומי מנדטים/אחוזים, כפילות רכה (אותם תאריכים ומספרים תחת מספר סימוכין אחר), חריגה >5 נק' מהסקרים הקודמים
של אותו מכון. סקר עם דגל נרשם ב-`db/quarantine.json` כ-`pending`,
נשאר מחוץ למודל, ונפתח Issue "[QA] סקר NNNN" - עם חוות דעת של "מבקר" Claude שמשווה את החילוץ לתמונת העמוד (מידע לאדם שמכריע, לא דגל בפני עצמו). תגובה ב-Issue שמתחילה ב-**אישור** או **דחייה** (workflow `qa_approve.yml`)
מעדכנת את הקובץ, סוגרת את ה-Issue ומריצה את המודל מחדש. ידנית: `python scraper/qa_check.py --approve 4135 --note "..."`.
רשת ביטחון נוספת ב-`db/build_db.py` (`fix_party`): מיפוי דטרמיניסטי של שמות חד-משמעיים (התאחדות הספרדים -> ש"ס, הרשימה הערבית המאוחדת -> רע"ם, חד"ש-תע"ל -> הרשימה המשותפת, המחנה הממלכתי -> כחול לבן).
האתר מציג בסעיף "איך זה עובד" את כל הסקרים שלא במודל והסיבה.

### בריף לטלגרם אחרי כל ריצה
`scraper/brief.py --send` מייצר בריף פרטי (מה נכנס, מה השתנה מול הריצה הקודמת ומול לפני שבוע, הסף, בדיקת תקינות), טיוטה לפרסום
