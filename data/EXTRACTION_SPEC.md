...
3. `pct`: the percentage shown for that party in the national/total column, as a number (17.7 not "17.7%"). If the table has several pct columns (e.g. per sector, raw vs weighted), use the national weighted one if present; otherwise the national raw one and say so in `notes`. null if there is no pct column.
   **Read the column headers carefully.** If ALL the percentage columns are sub-samples (e.g. "מדגם בוגרים יהודים", "מדגם צעירים", "מדגם חרדים", "מגזר ערבי", "לא משוקללים") and there is no national/total percentage column, then there is NO national pct: set `pct: null` for every row and use only the seats column ("מנדטים"), and write in `notes`: "pct columns are per-sector raw samples; only seats are national". Never put a sector column (such as the Jewish-adults sample) into `pct` - it is not the national share (a common layout of פרויקט המדגם/רוזנר).
